...
- **Batch 내 retrieval 정확도가 ~9% 부근** — 작아 보이지만 random 1/256=0.39% 대비 24배. 다만 같은 배치에 동일 캡션이 여러 번 반복 등장하는 구조라 (10 템플릿 × 10 클래스 = 100개 유니크 캡션 × 256 batch), 정답이 여러 개라 해석이 모호한 지표. 실제 능력 측정은 zero-shot 정확도가 더 정확
